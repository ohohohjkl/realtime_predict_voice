...
//hyper_vector DFT_power(float * fftBuffer, long fftFrameSize, long sign,int size)
//{
//	float wr = 0, wi = 0, arg = 0, temp = 0;
//	float tr = 0, ti = 0, ur = 0, ui = 0;
//	long i, bitm, j, le, le2, k;
//	hyper_vector pow_spectrum;
//	pow_spectrum.data = malloc(sizeof(SAMPLE) * size * (fftFrameSize / 2 + 1));
...
hyper_vector get_feature_vector_from_signal(SIGNAL a)
{
	...
	//writeDBFS(a.raw_signal, 0, a.signal_length);
...
{
	hyper_vector pow_spectrum;
	pow_spectrum.data = malloc(sizeof(SAMPLE) * frames.row * n);
	...
	for (int i = 0; i < frames.row; i++) {
		...
		for (int j = 0; j < frames.col; j++)
			printf("%f ", frames.data[i*n + j]);
	}
	for (int i = 0; i < frames.row; i++) {
		for (int j = 0; j < bound; j++) {
			if (j < frames.col)
				out[j].real = buf[j].real = frames.data[i*n + j];
			else
		...
		for (int j = 0; j < n; j++) {
			...
			pow_spectrum.data[i* n + j] = temp * temp / frames.col;
		}
	}
	free(buf);
	free(out);
