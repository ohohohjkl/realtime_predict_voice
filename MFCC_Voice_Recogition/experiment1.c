...
//	float sum;
//	for (k = 0; k < a.row; k++) {
//		for (i = 0; i < num_ceps; i++) {
//			sum = 0;
//			for (j = 0; j < len; j++)
//				sum += a.data[k*len + j] * cos((j + 0.5) * i * factor);
//			dct[k*num_ceps + i] = sum;
//			printf("%f ", sum);
//		}
//		printf("\n");
//
...
//	SIGNAL temp;
//	temp.raw_signal = (float *)malloc(sizeof(float) * size);
//	memcpy(temp.raw_signal, a, sizeof(float) * size);
//	/*for (int i = 0; i < size; ++i) {
//	temp.raw_signal[i] = a[i];
//	}*/
//	temp.frame_length = SAMPLE_RATE * 0.025;
...
//	int temp = frame_step;
//
//	SAMPLE *frames = (SAMPLE*)calloc(a.num_frame * a.frame_length, sizeof(SAMPLE));
//
...
//*/
//
//hyper_vector get_feature_vector_from_signal(hyper_vector a, hyper_vector fbank, hyper_vector temp_feats,int numFrames, int comFlag)
//{
...
//
//
//	append_to_final(temp_feats, numFrames);
//
//	if (comFlag) {
...
//}
//
//void add_to_final(hyper_vector temp_feats, int numFrames) {
//
//}
//
...
//			cplx t = mul(cexp(PI * i / n), out[i + step]);
//			buf[i / 2] = add(out[i], t);
...
//		pow_spectrum.data[j] = temp * temp / frames.col;
//	}
//	temp = 0;
//	free(cx_in);
//	free(cx_out);
...
//	long size = temp.col*temp.row*temp.dim;
